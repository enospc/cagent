...
static MONITORING_DISABLED: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
...
impl Utils {
    ...
    pub fn log_command_start(&self, cmd: &Command) {
        ...
        if self.config.verbose_mode {
            println!("{}[EXEC]{} {}", crate::constants::BLUE, crate::constants::NC, cmd_string);
        }
    ...
    pub fn log_command_output(&self, level: &str, source: &str, line: &str) {
        ...
        if self.config.verbose_mode {
            ...
            println!("{}[{}]{} {}", color, source.to_uppercase(), crate::constants::NC, line);
        }
    ...
    pub fn log_command_end(&self, cmd: &Command, success: bool, duration: Duration) {
        ...
        let message = format!("Completed: {} - {} (took {:.2}s)", cmd_string, status, duration.as_secs_f64());
        ...
        if self.config.verbose_mode {
            ...
            println!("{}[{}]{} {} (took {:.2}s)", color, status, crate::constants::NC, cmd_string, duration.as_secs_f64());
        }
    ...
    pub fn run_command_with_realtime_logging(&self, cmd: &mut Command, error_msg: &str) -> Result<(), String> {
        self.run_command_with_realtime_logging_and_monitoring(cmd, error_msg, true)
    }


    fn run_command_with_realtime_logging_and_monitoring(&self, cmd: &mut Command, error_msg: &str, enable_monitoring: bool) -> Result<(), String> {
        self.log_command_start(cmd);
        let start = Instant::now();
        ...
        let mut child = cmd.spawn().map_err(|e| format!("{}: Failed to spawn process: {}", error_msg, e))?;
            ...
            Some(thread::spawn(move || {
                ...
                loop {
                    ...
                    if time_since_output > Duration::from_secs(10) && !wait_reported {
                        ...
                        utils.log_with_level("WAIT", &format!("Process appears to be waiting for input (no output for {:.1}s)", time_since_output.as_secs_f64()));
                        wait_reported = true;
                    }
        ...
        let exit_status = child.wait().map_err(|e| format!("{}: Failed to wait for process: {}", error_msg, e))?;
        let duration = start.elapsed();
        ...
        if !success {
            ...
            return Err(format!("{}: Process exited with code {}", error_msg, exit_code));
        }
    ...
    pub fn run_systemd_nspawn_with_network(&self, args: &[&str], error_msg: &str) -> Result<(), String> {
        let mut cmd = Command::new("sudo");
        cmd.arg("systemd-nspawn");
    ...
    pub fn run_systemd_nspawn_with_output(&self, args: &[&str], error_msg: &str) -> Result<String, String> {
        let mut cmd = Command::new("sudo");
        cmd.arg("systemd-nspawn");
        ...
        if !output.status.success() {
            ...
            return Err(format!("{}: Command failed with stderr: {}", error_msg, stderr));
        }
